...
if FROZEN:
    ...
    if w3x[w3x.rfind('.'):] == '.j':
        ...
        w3x = mpq.MPQEditor('MPQEditor', w3x)

        w3x.extract('war3map.j', DIRECTORY+'war3map.j').wait()

        with open(DIRECTORY+'war3map-opt.j','w') as f:
            do(DIRECTORY+'war3map.j\\war3map.j', f)

        w3x.delete('war3map.j').wait()
        w3x.flush().wait()

        w3x.add(DIRECTORY+'war3map-opt.j', 'war3map.j').wait()
        w3x.extract('war3map.j', DIRECTORY+'war3map-new.j').wait()
